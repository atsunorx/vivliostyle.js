describe("vivliostyle.plugins.hyphenation", function() {
    describe("ForbidHyphenationAtTheEndOfColumnsTextNodeBreaker", function() {
        ...
        describe("#breakTextNode", function() {
            ...
            it("removes characters after the word boundary before the soft hyphen when splits a text node at the soft-hyphen character.", function() {
            ...
            it("removes characters after the word boundary before the hyphen when splits a text node at the hyphen character.", function() {
            ...
            it("returns null when splits a text node at the soft hyphen character in the first word.", function() {
                textNode.data = 'abcdeabcde\u00ADf gh\u00ADj';
                var newContext = breaker.breakTextNode(textNode, nodeContext, 13, checkPoints, 1);
                expect(newContext).toEqual(null);
                expect(textNode.replaceData).not.toHaveBeenCalled();
            });
            it("returns null when splits a text node at the hyphen character in the first word.", function() {
                textNode.data = 'abcdeabcde-f gh-j';
                var newContext = breaker.breakTextNode(textNode, nodeContext, 13, checkPoints, 1);
                expect(newContext).toEqual(null);
                expect(textNode.replaceData).not.toHaveBeenCalled();
            });
            it("removes characters after split point when splits a text node at the space character.", function() {
